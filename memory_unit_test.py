...
if __name__ == '__main__':
    ...
    for i in range(repeat):
        ...
        if i == 0:
            for g in cur_graphs:
                graph_sizes.append(len(g.vertices))
        ...
        for g in cur_graphs:
            ...
            for q in cur_queries:
                ...
                solver.search()
                memory_array.append(tracemalloc.get_traced_memory())
            memory_arrays.append(memory_array)
        tracemalloc.stop()
        memory_arrays_repeat.append(memory_arrays)

    graphs_sizes = np.array(graph_sizes)
    memory_arrays_repeat = np.array(memory_arrays_repeat)

    if not os.path.exists('./output'):
        os.makedirs('./output')

    np.save('output/memory_unit_test_{}'.format(test_name), memory_arrays_repeat)
